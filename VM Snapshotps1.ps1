$Subs = @("Microsoft Azure Enterprise Travelzoo-Inc";"Microsoft Azure Enterprise Travelzoo-Pub";"Microsoft Azure Enterprise Travelzoo-Pub-Legacy")
$tagname = 'Updates'
...
foreach ($Sub in $Subs) {
    ...
    write-Output "gathering all VM's with tag Updates $weekinmonth"
    ...
    $ListToSnapshotRel = Get-AzResource -TagName $tagname -TagValue "0"
    write-Output "gathering all VM's with tag Updates:0"
	Foreach ($VMrel in $ListToSnapshotRel) {
	    $resourceGroupName = $VMrel.ResourceGroupName
	    $location = $VMrel.Location
	    $vmName = $VMrel.name
	    $snapshotName = $VM.name+"$(get-date -Format 'yyyy-MM-dd')"
	    $Temp = (Get-date).AddDays(2)
	    $date = get-date $Temp -Format 'yyyy-MM-dd'
	    $tag = @{"Remove after"="$date"}
